...
func (ic *IdentityProviderConfig) oauthProvider(pc ssokenizer.ProviderConfig, c *Config) (ssokenizer.Provider, error) {
	...
	case "google":
		...
		op.ForwardParams = []string{"hd", "prompt"}
		return &op, nil
	case "heroku":
	...
	case "slack":
		...
		op.CustomExchange = slackExchange
		op.CustomRefresh = slackRefresh
		return &op, nil
	case "squadcast":
		op.OAuthConfig.Endpoint = xoauth2.Endpoint{
			AuthURL:  "https://auth.squadcast.com/oauth/authorize",
			TokenURL: "https://auth.squadcast.com/oauth/access-token",
		}
		op.CustomRefresh = squadcastRefresh
		return &op, nil
	default:
